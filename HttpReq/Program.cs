...
namespace HttpReq
{
    class Program
    {
        ...
        static StringContent stringContent = new StringContent(File.ReadAllText(@"E:\Perf\input10KB.txt"));
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"50th latency percentile: {(int)Percentile(_latencies.ToArray(), 0.50)}ms");
            Console.WriteLine($"90th latency percentile: {(int)Percentile(_latencies.ToArray(), 0.90)}ms");
        ...
        static async Task SendRequestsAsync(string url, int maxIterations, int countPerIteration, int delay, bool clientMode)
        {
            ...
            else
            {
                ...
                for (int currentIteration = 0; currentIteration < maxIterations; currentIteration++)
                {
                    ...
                    {
                        tasks.Add(
                            GetHttpClient().PostAsync(url, stringContent).ContinueWith(
        ...
        static async Task SendRequestsAsyncOneClient(string url, int maxIterations, int delay, int clientIndex)
        {
            for (int currentIteration = 0; currentIteration < maxIterations; currentIteration++)
            {
                ...
                await RequestDone(
                    GetHttpClient().PostAsync(url, stringContent),
                    invocationState
                );
        ...
        static async Task RequestDone(Task<HttpResponseMessage> action, object state)
        {
            ...
            if (action.Result.StatusCode == HttpStatusCode.Accepted || action.Result.StatusCode == HttpStatusCode.OK)
            {
                return;
            }
